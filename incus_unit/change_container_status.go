...
func ChangeState(tag string, newState string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed to request Incus %s: %w", newState, err)
	}
	...
	if err != nil {
		...
		return fmt.Errorf("failed to wait for Incus %s: %w", newState, err)
	}
	...
	if err != nil {
		...
		return fmt.Errorf("failed to update container status to %s in DB: %w", newState, err)
	}
